...
for dep, sem in all_classes:
    ...
    all_courses = [(x[0], x[4], x[5], x[7]) for x in show_courses_department_wise_lec(dep, sem)]
    ...
    for course in all_courses:
        ...
        while course[2] != show_course_with_rid(course[0])[-1]:
            ...
            time_slots_local = [x for x in range(len(time_slots)) if x not in [4,8]]
            time_slot = random.choice(time_slots_local)
            
            if  (not is_occupied(course[1], day, time_slot)) and (not is_class_occupied(dep, sem, day, time_slot)) and (not is_occupied(course[1], day, time_slot + 1)) and (not is_class_occupied(dep, sem, day, time_slot + 1)):  
...
for dep, sem in all_classes:
    ...
    all_courses = [(x[0], x[4], x[5]) for x in show_courses_department_wise_lec(dep, sem)]
    working_days_local = working_days.copy()
    time_slots_local = time_slots.copy()
